...
1. **查板汇报**：优先运行 `python3 <agent-board skill 目录>/scripts/board_overview.py`（人类专用全局视图：总进度/在飞 agent/逐单最新留言；`--todo` 只看未完结），辅以 `feed -n 20`。用中文汇报：各状态任务数、进行中任务（谁在做、多久没活动、是否可 steal）、未回应的留言（尤其 boss 自己的）。
